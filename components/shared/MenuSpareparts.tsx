...
export default function MenuSpareparts({ category, subcategory }: CardProps) {
  ...
  return (
    <div className="mt-2">
      <div className="grid grid-cols-5 lg:grid-cols-8 m-0 gap-1">
        {automotivePartsCategories.length > 0 &&
          automotivePartsCategories.slice(0, 8).map((vehicle: any) => (
            <div
              key={vehicle.name} // Always good to have a unique key prop
            ...
            >
              <div
                ...
                onClick={(e) => setQuery(vehicle.name)}
              >
                ...
                <h2 className="text-xs">{vehicle.name}</h2>
              </div>
            </div>
          ))}
      </div>
